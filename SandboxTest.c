...
JNIEXPORT jint JNICALL Java_SandboxTest_syscallOpen
  (JNIEnv *env, jobject this, jstring path, jint oflags)
{
	//printf("Java_SandboxTest_syscallOpen()... ");
	const char *nativePath = (*env)->GetStringUTFChars(env, path, 0);
	//printf("calling open(\"%s\", %d, 0766)...", nativePath, oflags);
	jint fd = open(nativePath, oflags, 0766);
	//printf("%d returned, releasing string...", fd);
